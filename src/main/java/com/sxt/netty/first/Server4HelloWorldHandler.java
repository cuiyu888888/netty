 ...
 *
 * 注意：当时用Sharable时，这就是一个共享的Handler，多个客户端共用一个Handler，这就会产生一个问题？
 * 		当在Handler定义一个全局的变量的时候，每个客户端都可以更改这个值，就会造成线程不安全，定避免定义可写的实例变量。
 */
@Sharable
public class Server4HelloWorldHandler extends ChannelHandlerAdapter {
	 ...
	 * 		用于处理读取数据请求的逻辑。
	 * 		ctx：
	 * 			上下文对象。其中包含于客户端建立连接的所有资源。 如： 对应的Channel
	 * 		msg：
	 * 			读取到的数据。 默认类型是ByteBuf，是Netty自定义的。是对ByteBuffer的封装。 不需要考虑复位问题。
	 */
	@Override
